...
int main(int argc, char *argv[]) {
    ...
    char *token;
    char firstQuery[] = "Insert first number";
    char secondQuery[] = "Insert second number";
    ...
    for(;;) {
        ...
        x = atoi(readLine);
        bzero(readLine, BUFSIZ);
        if(sendto(sockfd, secondQuery, strlen(secondQuery), 0, (struct sockaddr*) &cli_addr, sizeof(cli_addr)) < 0){
            error("ERROR : sendto()");
        }        
        bytesRead = recvfrom(sockfd, readLine, BUFSIZ, 0, (struct sockaddr*) &cli_addr, &clilen);
